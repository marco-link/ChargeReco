...
for path in glob.glob('crab_*/*'):
    ...
    # job complete
    if status['Status on the scheduler'] == 'COMPLETED' and '100.0%' in status['Publication status']:
        Ncomplete +=1
    ...
    # publishing jobs
    elif status['Status on the scheduler'] == 'COMPLETED':
        Npublishing +=1
        message = BOLD + 'PUBLISHING  \t' + message + status['Publication status'] + RESET
...
print(BOLD)
print('+++ DONE +++')
print('{} total jobs'.format(Ncomplete + Npublishing + Nrunning + Nresubmitted + Nfailed))
print('{} complete jobs'.format(Ncomplete))
print('{} publishing jobs'.format(Npublishing))
